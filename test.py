...
def main():
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        for val_data in val_loader:
            ...
            # 以下变量用于计算LPIPS
            img = visuals['rlt'].squeeze(0).cuda()
            gt = visuals['GT'].squeeze(0).cuda()
            ...
            if save_imgs:
                try:
                    ...
                    cv2.imwrite(osp.join(output_folder_s1, tag), rlt_s1_img)
            ...
            lpips = loss_fn_alex(img, gt)
            lpips = lpips.item()
            lpips_rlt[folder].append(lpips)
