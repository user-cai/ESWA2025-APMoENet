...
class enhancement_model(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        self.fake_H, self.fake_H_s1, self.fr, self.fr_refine, self.output1_edge = self.netG(self.var_L, self.fr)
        ...
        l_pix_amp = self.l_pix_w * self.cri_pix(self.fake_H_s1, self.real_H) * 0.1
        l_vgg = self.l_pix_w * self.cri_vgg(self.fake_H, self.real_H) * 0.3
        self.fr_refine = (self.fr_refine - self.fr_refine.min()) / (self.fr_refine.max() - self.fr_refine.min())
        l_ssim = (1 - self.ssim_loss(self.fake_H, self.real_H)) * 0.3
        l_ssim_s1 = (1 - self.ssim_loss(self.fake_H_s1, self.real_H)) * 0.1
        l_edge1 = edge_consistency_loss(self.real_H, self.output1_edge) * 0.1
        l_edge2 = edge_consistency_loss(self.real_H, self.fr_refine) * 0.1
        l_edge = l_edge1 + l_edge2

        l_final = l_pix  + l_pix_amp + l_ssim + l_vgg + l_ssim_s1 + l_edge
        l_final.backward()
        ...
        self.log_dict['l_ssim_s1'] = l_ssim_s1.item()
        self.log_dict['l_edge'] = l_edge.item()

    def test(self):
        self.netG.eval()
        with torch.no_grad():
            self.fake_H, self.fake_H_s1 ,self.fr, _, _ = self.netG(self.var_L, self.fr)
        self.netG.train()
